...
The correlation plot above indicates relatively high correlation between uniformity of cell size and uniformity of cell shape (a value of 0.91). Going forward, I will create several models: some using the uniformity of cell shape variable and some using the uniformity of cell size variable.
...
I will build logistic regression models using 10-fold cross validation and the LASSO shrinkage method. The first model will contain uniformity in cell shape while the second will contain uniformity in cell size.
...
#### Ridge Regression
I wil also build logistic regression models using 10-fold cross validation and the ridge regression shrinkage method. The first model will contain uniformity in cell shape while the second will contain uniformity in cell size.
...
The optimal lambda value for model 3 gives us the following coefficients for the model:

```{r}
coef(model3$finalModel, model3$bestTune$lambda)
```

There appears to be a positive relationship between every dependent variable and malignant tumor status. The single epithelial cell size was determined to not have an influence on determining malignant tumors. This model has a `r round(max(model3$results$Accuracy) * 100, 2)`% accuracy on the training data.

The optimal lambda value for model 4 gives us the following coefficients for the model:

```{r}
coef(model4$finalModel, model4$bestTune$lambda)
```

There appears to be a positive relationship between every dependent variable and malignant tumor status. This model has a `r round(max(model4$results$Accuracy) * 100, 2)`% accuracy on the training data.

## Choosing a Final Model

It appears that model 2 has the highest accuracy at `r round(max(model2$results$Accuracy) * 100, 2)`%, which means I will be selecting model 2 (using LASSO shrinkage and containing the uniformity in cell size variable).
