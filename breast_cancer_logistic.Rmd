...
It appears that ridge logistic regression model has the highest accuracy at `r round(max(ridge$results$Accuracy) * 100, 2)`%, which means I will be selecting this model going forward.
...
testing_prediction = predict(ridge, newdata = test_new, se = T)
...
The ridge logistic regression model has an accuracy of 95.24% in predicting tumor status. The model has a recall of 99.25% and a precision of 93.66%. This shows that the model has done fairly well in identifying if tumors are benign or malignant. 
